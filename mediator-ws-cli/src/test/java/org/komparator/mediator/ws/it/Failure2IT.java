 ...
 * Test suite
 */
public class Failure2IT extends BaseIT {
	...
	,InvalidQuantity_Exception, NotEnoughItems_Exception {
		...
		cart = cartlist.get(0).getItems().get(0);
		assertEquals(5,cart.getQuantity());


		ShoppingResultView result = mediatorClient.buyCart("cart","6769381066990468");
		ShoppingResultView result2 = mediatorClient.buyCart("cart","6769381066990468"); 
		List<ShoppingResultView> list = mediatorClient.shopHistory();
		assertEquals(result.getId(),result2.getId()); //id é único, verifica maximo uma vez
